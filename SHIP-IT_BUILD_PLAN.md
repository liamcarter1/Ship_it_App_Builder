...
**Milestone 4 — Polish & reuse. ✅ DONE.** The `GatePanel` now dispatches to per-gate body renderers: a structured `SpecView` for the spec gate, a line-coloured `DiffView` (over `git diff HEAD` against the scaffolder commit, capped at 40 KB) for the code gate, and a confirmation `DeployView` for the deploy gate. The `NewRunForm` exposes per-worker model overrides in an *Advanced* disclosure that threads through `POST /api/runs` to `OrchestratorConfig`. The run detail page has a *Cancel run* button that hits `POST /api/runs/{id}/cancel`. `Store.decode_event(row)` consolidated the event-row JSON parse used by both the FastAPI server and the CLI `--show-run` replay.

**Milestone 5 — Restart-resumable, DB-backed gates. ✅ DONE.** Gate state moved from in-process `asyncio.Future`s to a SQLite `gates` table; `GateBroker` polls the table every ~250ms for a decision (the same "poll SQLite" mechanism the SSE stream uses — so restart-durability and multi-worker correctness fall out of one design). Run config (deploy flag, `max_review_rounds`, five `*_model` overrides) persisted in a `config` JSON column on `runs`. `Orchestrator.resume_tail` re-enters the pipeline at the gate it was paused on. A startup sweep `_recover_runs()` in the FastAPI `lifespan` claims each `'running'` run atomically (guards against double-resume), resumes code/deploy-gated runs via `resume_tail`, and marks spec-gate or mid-stage runs `'interrupted'`. Two new event kinds: `pipeline_resumed`, `run_interrupted`. Design spec: `docs/superpowers/specs/2026-05-31-restart-resumable-gates-design.md`; plan: `docs/superpowers/plans/2026-05-31-restart-resumable-gates.md`. Non-goal: resuming a run that died mid-LLM-stage.

**Post-M5 — Preview-locally. ✅ DONE.** The run detail page can launch a finished run's generated app (`built` / `deployed` / `deploy_failed`) with `npm run dev` and surface a clickable `localhost` link, with a Stop button. A `PreviewManager` (`backend/app/preview.py`) owns at most one preview at a time — it picks a free port in 4300–4399, spawns the dev server off the event loop, TCP-polls for readiness, and persists the PID/port in a one-row `previews` table so a startup sweep (`_preview.recover()`) can reap an orphan after a crash; a background idle reaper stops it after 30 min. Endpoints: `POST/GET/DELETE /api/runs/{id}/preview`; UI in `frontend/components/PreviewPanel.tsx`. The Windows process-tree kill helper was extracted to shared `backend/app/proc.py`. Design spec: `docs/superpowers/specs/2026-06-03-preview-locally-design.md`; plan: `docs/superpowers/plans/2026-06-03-preview-locally.md`.
...
All five planned milestones are done. The restart-resumable / DB-backed gates work (Milestone 5) is complete — see the design spec at `docs/superpowers/specs/2026-05-31-restart-resumable-gates-design.md` and the plan at `docs/superpowers/plans/2026-05-31-restart-resumable-gates.md`.

The one remaining non-goal is **mid-LLM-stage resume** (resuming a run that died while a Coder/Reviewer `query()` was in flight). In-flight agent calls cannot be replayed; such runs are correctly marked `'interrupted'` and can be restarted fresh from the dashboard. If you need full mid-stage durability in a future version, that would require checkpointing the agent's turn stream to the DB — a significantly larger change.
